...
$(function () {
    ...
    _t_end= nian+"-"+yue+"-"+ri;
	 ...
	 $('#bkyxtjGrid').datagrid({    
		    url:'<%=request.getContextPath()%>/personInfo/school?year1='+_t_start+'&year2='+_t_end,  
		     border:true,
			pageSize:10,
		    ...
		    columns:[[    
		        ...
		        {field:'school',title:'学校',width : $(this).width() * 0.4,align:'center'},    
		        {field:'num',title:'数量',width : $(this).width() * 0.4,align:'center'}
		  
		    ]]    
		});  
			var pager = $('#bkyxtjGrid').datagrid('getPager');
			pager.pagination({
    ...
    _t_end= nian+"-"+yue+"-"+ri;
		...
		$.ajax({
		type:"POST", 
		url:"/jtrc/personInfo/school",
		async:false,
		data:{year1:_t_start,year2:_t_end},
			success:function(result){
			console.log(result);
				for(var i = 0 ;i<result.length;i++){
				school[i]=result[i].school;
				num[i]=result[i].num;
				}
	 		},
		});
...
<span style="color:red">(如有slice请先点灰)</span>
