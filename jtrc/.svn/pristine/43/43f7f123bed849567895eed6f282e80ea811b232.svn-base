...
$(function () {
    ...
    _t_end= nian+"-"+yue+"-"+ri;
	 ...
	 $('#bkzxtjGrid').datagrid({    
		    url:'<%=request.getContextPath()%>/personInfo/major?year1='+_t_start+'&year2='+_t_end,  
		     border:true,
			pageSize:10,
		    ...
		    columns:[[    
		        ...
		        {field:'major',title:'专业',width : $(this).width() * 0.4,align:'center'},    
		        {field:'num',title:'数量',width : $(this).width() * 0.4,align:'center'}
		  
		    ]]    
		});  
			var pager = $('#bkzxtjGrid').datagrid('getPager');
			pager.pagination({
    ...
    _t_end= nian+"-"+yue+"-"+ri;
		...
		$.ajax({
		type:"POST", 
		url:"/jtrc/personInfo/major",
		async:false,
		data:{year1:_t_start,year2:_t_end},
			success:function(result){
			console.log(result);
				for(var i = 0 ;i<result.length;i++){
				major[i]=result[i].major;
				num[i]=result[i].num;
				}
	 		},
		});
