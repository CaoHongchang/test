...
$(function () {
    ...
    _t_end= nian+"-"+yue+"-"+ri;
	 ...
	 $('#xxxstjGrid').datagrid({    
		    url:'<%=request.getContextPath()%>/personInfo/formsOfLearning?year1='+_t_start+'&year2='+_t_end,  
		     border:true,
			pageSize:10,
		    ...
		    columns:[[    
		        ...
		        {field:'learnType',title:'类型',width : $(this).width() * 0.4,align:'center'},    
		        {field:'num',title:'数量',width : $(this).width() * 0.4,align:'center'}
		  
		    ]]    
		});  
			var pager = $('#xxxstjGrid').datagrid('getPager');
			pager.pagination({
    ...
    _t_end= nian+"-"+yue+"-"+ri;
		...
		$.ajax({
		type:"POST", 
		url:"/jtrc/personInfo/formsOfLearning",
		async:false,
		data:{year1:_t_start,year2:_t_end},
			success:function(result){
			console.log(result);
				for(var i = 0 ;i<result.length;i++){
				learnType[i]=result[i].learnType;
				num[i]=result[i].num;
				}
	 		},
		});
