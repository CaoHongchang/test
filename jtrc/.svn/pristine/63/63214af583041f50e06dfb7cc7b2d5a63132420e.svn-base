...
@Service
public class ArchivesOutInfoServiceImpl implements
ArchivesOutInfoService {
	private static Logger logger = Logger.getLogger(ArchivesOutInfoService.class);
	@Autowired
	...
	@Override
	public int insertArchivesOutInfoSelective(ArchivesOutInfo record,String fjparam,String ptype)throws Exception{
		// TODO Auto-generated method stub
		//为新增材料添加排序字段sort的数值，因原有数据没有sort字段故而有可能出现为空值的情况。
		Map<String,Object> map=new HashMap<String, Object>();
		map.put("pno", record.getpNo());
		Integer sort;
		if(archivesOutInfoMapper.getsort(map)!=null&&archivesOutInfoMapper.getsort(map).get("sort")!=null)
			sort =Integer.valueOf(archivesOutInfoMapper.getsort(map).get("sort").toString())+1;
		else
			sort=0;
		record.setSort(sort);
		int c= archivesOutInfoMapper.insertSelective(record);
		if(c>0){
			//新增附件列表
	...
	@Override
	@Transactional
	public int updateArchivesOutInfoSelective(ArchivesOutInfo record,String fjparam,String ptype)throws Exception{
		// TODO Auto-generated method stub
		logger.info("====开始做修改操作，修改的参数为：record"+record+"fjparam="+fjparam+"ptype="+ptype);
		int c= archivesOutInfoMapper.updateByPrimaryKeySelective(record);
		if(c>0){
			ArchivesMaterialOtherInfo a=new ArchivesMaterialOtherInfo();
			if(StringUtils.isNotEmpty(ptype))
				a.setFileType(Integer.valueOf(ptype));//材料类型（1：履历材料 2：自传材料 3：鉴定考核考察材料 4：鉴定考核考察材料 5：政审材料 6：党员材料 7：奖励材料 8：处分材料9：工资变动材料 10：其他材料）
	...
	@Override
	public List<Map<String,Object>> listArchivesOutInfos(ArchivesOutInfo record) {
		// TODO Auto-generated method stub
		return archivesOutInfoMapper.selectByPno(record);
	}
}
