...
<head>
    ...
    <script type="text/javascript" src="<%=request.getContextPath()%>/ps/sjzhgl.js"></script>
...
function saveOrUpdateProductNode(){
	...
	var name = $("#name1").val();
	   ...
	   url = "../shoptype/insert?title="+$("#name1").val();
	}else{
		var name = $("#name1").val();
	 	if(name==""){
	 	alert("商户类别不能为空");
		...
		var row = $('#noticeRs').datagrid('getSelected');
	   url = "../shoptype/update?title="+$("#name1").val()+"&id="+row.id;
	}
	$.post(url,null,function(data){
		if(data.flag==1){
			alert("发布成功！");
