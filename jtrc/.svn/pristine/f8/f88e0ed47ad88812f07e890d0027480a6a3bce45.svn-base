...
//保存
function saveYwjbrFuc(){
    if(sessionUserId==null||sessionUserId==''){
    	$.messager.alert('提示','用户未登录无法操作！');
			return false;
    
    }
    var r = $("#ywjbrForm").form('enableValidation').form('validate');
	if(!r) {
		return false;
	}

 var param=$("#ywjbrForm").serializeArray();
 param=JSON.stringify(param); 
 
$("#ywjbrForm").ajaxSubmit({ 
...
$('#sstate').combobox({    
   ...
   <input type="hidden" id="id" name="id">
    <table class="table3">
      ...
      <select id="sstate" class="easyui-combobox" name="sstate" style="width:375px;"data-options="editable:false" >  
