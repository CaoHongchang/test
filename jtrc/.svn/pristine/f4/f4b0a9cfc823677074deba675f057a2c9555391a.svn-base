       ...
       <table  class="table2">
	     <tr>
	       <td class="left">审核状态</td>
	        <td class="right">
	            <select id="trainCheckState" class="easyui-combobox" name="trainCheckState" style="width:135px;">   
		 ...
		 <tr>
	       ...
	       <td class="right">
	         <input class="easyui-textbox" name="trainRemark" id="trainRemark" size="40" data-options="multiline:true"  style="height:52px">
		   </td>
		 </tr>
		 <tr>
	       <td class="left">培训信息</td>
	       <td class="right">
	         <input class="easyui-textbox" name="trainMsg" id="trainMsg" size="40" data-options="multiline:true"  style="height:52px">
		   </td>
		 </tr>
	   </table>   
      </div>
...
<script>
$("#auditForm #trainCheckState").combobox({
	onChange: function (n,o) {
	    if(n == '2'){
            $("#trainMsg").textbox('setValue','');  //设置输入框的值
            $('#trainMsg').textbox('textbox').attr('readonly',true);  //设置输入框为禁用
        }else{
        	$('#trainMsg').textbox('textbox').attr('readonly',false);  //设置输入框为禁用
        }
	}
})
</script>
