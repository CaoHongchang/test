...
function ldshFuc(param){

   $('#ldshglGrid').datagrid({    
    url:basePath+"ywslsh/ldshView",
     border:true,
	pageSize:10,
    ...
    columns:[[    
        ...
        {field:'ywmc',title:'业务名称',width: fixWidth(0.1),align:'center'},
        ...
        {field:'zdldshr',title:'指定领导审核人',width: fixWidth(0.1),align:'center'},
/*      {field:'ywjbr',title:'业务经办人',width: fixWidth(0.3),align:'center'}, */ 
       {field:'ldshzt',title:'领导审核状态',width: fixWidth(0.1),align:'center'},
        {field:'bhyj',title:'领导审核意见',width: fixWidth(0.3),align:'center'},
         ...
         {title:'操作',field:'id',width: fixWidth(0.1),align:'center',
		    		...
		    		if(ywsl&&ck)
 				      return '<a href="#" style="color: green;" id="ywsl" onclick="addldshFuc(\''+row.id+'\',\''+row.parentId+'\')">业务受理</a> <a href="#" style="color: blue;" id="ck"  onclick="addywslshFuc('+value+')">查看</a>';
				
				   else if(!ywsl&&ck)
				    				      return '<a href="#" style="color: blue;" id="ck"  onclick="addywslshFuc('+value+')">查看</a>';
				    		else if(ywsl&&!ck)
				    		 				      return '<a href="#" style="color: green;" id="ywsl" onclick="addldshFuc(\''+row.id+'\',\''+row.parentId+'\')">业务受理</a> ';
				}   
				}
  
    ]]    
});  
	var pager = $('#ldshglGrid').datagrid('getPager');
	pager.pagination({
...
function searchldshDiv(){
   
    $('#ldshSearchInfo').dialog({    
    title: '',    
    width: 1000,    
    height: 300,    
    closed: false,    
...
//导出受理单
function sldExport(){
if(!isCheckOne("#ldshglGrid")){
    return;
 }
  	var rows = $('#ldshglGrid').datagrid('getSelections');
    var pno=rows[0].slbh;
    if(pno==null||pno==''){
    $.messager.alert('提示', '受理编号不能为空！');
       ...
       $.ajax({
                ...
                if(data.flag ==true){
                 var sstate=data.rows.sstate;
               	 if(sstate=='5'){//审核状态 0:已受理  1:驳回   2:已审核 3:审核失败4：未审核5:办结
...
if(!isCheckOne("#ldshglGrid")){
    ...
    var pno=rows[0].slbh;
    var id=rows[0].id;
    var ldshzt=rows[0].ldshzt;
    if(ldshzt!='已审核'){
     $.messager.alert('提示','领导审核，只有已审核的状态，才能进行办结操作!','info');
     return;
    }else{
    
    $.ajax({
                url: basePath+"ywslsh/ldshEnd?pno="+pno+"&id="+id,
                type: "POST",
                contentType : "application/json;charset=utf-8", 
                dataType:"json",
                async: false,
...
<!-- 		<a onclick="addywslshFuc();" class="easyui-linkbutton" data-options="iconCls:'icon-tip',plain:true">查看</a>-->
		...
		<a onclick="searchldshDiv();" class="easyui-linkbutton" data-options="iconCls:'icon-search',plain:true" id="cx">查询</a>
		<a onclick="sldExport();" class="easyui-linkbutton" data-options="iconCls:'icon-print',plain:true" id="dcsld">导出受理单</a>
		  <a onclick="endFuc();" class="easyui-linkbutton" data-options="iconCls:'icon-add',plain:true" id="jb">办结</a>
	</div>

</div>
<div>
<table id="ldshglGrid"></table>  
</div>
<div id="ldshInfo"></div>
<div id="xgbmInfo"></div>
<div id="ldshSearchInfo"></div>
