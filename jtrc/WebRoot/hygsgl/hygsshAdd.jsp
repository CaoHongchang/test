...
//保存
function saveHygsshFuc(){
    if(sessionUserId==null||sessionUserId==''){
    	$.messager.alert('提示','用户未登录无法操作！');
			return false;
    
    }
    var r = $("#hygsshForm").form('enableValidation').form('validate');
	if(!r) {
		return false;
	}

 var param=$("#hygsshForm").serializeArray();
 param=JSON.stringify(param); 
 
$("#hygsshForm").ajaxSubmit({ 
...
$('#sstate').combobox({    
   onSelect: function(rec){
	...
	<form id="hygsshForm" method="post">
   <input type="hidden" id="id" name="id">
    <table class="table3">
      ...
      <option value=""></option>
      <option value="4" selected="selected">相关部门</option>
