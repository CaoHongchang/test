	         ...
	         <table class="table2">
				  <tr>
				      ...
				      <td class="right fvalue" colspan="3">
				      	...
				      	<c:if test="${enterprise.status == 1 }">checked</c:if>
				      	class="easyui-validatebox" data-options="validType:['requireRadio[\'status\']']"  value="1"/>审核通过
				      	<input type="radio" name="status"
				      	<c:if test="${enterprise.status == 2 }">checked</c:if>
				      	 class="easyui-validatebox" data-options="validType:['requireRadio[\'status\']']"  value="2"/>审核不通过
				      </td>
				  </tr>
				  <tr>
				      <td class="left flabel">审核说明：</td>
			      	 <td class="right" colspan="3"><input class="easyui-textbox" 
			      	 value="${enterprise.reason }"
			      	 name="reason" data-options="multiline:true" style="height:60px;width:600px;"></input></td>
				  </tr>
		  	 </table>
		</div>
	</div>
