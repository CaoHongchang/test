...
public class ToolDateTime {
    ...
    public static List<String> getDaySpaceDate(Date start, Date end) {
        ...
        long dayCount = (toCalendar.getTime().getTime() - fromCalendar
                .getTime().getTime())
                / (1000 * 60 * 60 * 24);
        if (dayCount < 0) {
            return dateList;
     ...
     *
     * @author Cary Huang
     * @created 2016年1月21日 上午10:20:18
     * @param date
     * @return
     */
    public static Map<String, Date> getStartEndWeekByDate(Date date){
        Calendar calendar = Calendar.getInstance();
        calendar.setTime(date);
    ...
    /**
     * 
     * 根据传入的当前时间，获取当前时间的开始秒和结束秒
     *
     * @author Charlotte Chen
     * @created 2016-5-24 下午5:36:15
     * @param date
     * @return
     */
    public static Map<String, Date> getStartEndByDate(Date date){
        Calendar calendar = Calendar.getInstance();
        calendar.setTime(date);
        Map<String, Date> map = new HashMap<String, Date>();
        //根据日历的规则，给当前日期减去星期几与一个星期第一天的差值   
     ...
     * 
     * @author Gerald Gu
     * @created 2014-12-9 下午03:40:01
     * @param bDate
     * @param eDate
     * @return
     */
    public static Integer dateCompare(Date bDate, Date eDate) {
        Integer flag = 0;
    ...
    public static void main(String[] args) throws ParseException {
        Map<String, Date> map = getStartEndWeekByDate(new Date());
        System.out.println(format(map.get("start"), PATTERN_YMD_HMS));
        System.out.println(format(map.get("end"), PATTERN_YMD_HMS));
        Map<String, Date> maps = getStartEndByDate(new Date());
        System.out.println(format(maps.get("start"), PATTERN_YMD_HMS));
        System.out.println(format(maps.get("end"), PATTERN_YMD_HMS));
