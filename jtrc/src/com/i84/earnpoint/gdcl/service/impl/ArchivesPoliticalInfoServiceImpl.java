...
@Service
public class ArchivesPoliticalInfoServiceImpl implements
ArchivesPoliticalInfoService {
	private static Logger logger = Logger.getLogger(ArchivesPoliticalInfoService.class);
	@Autowired
	...
	@Override
	public int insertArchivesPoliticalInfoSelective(ArchivesPoliticalInfo record,String fjparam,String ptype) {
		// TODO Auto-generated method stub
		//为新增材料添加排序字段sort的数值，因原有数据没有sort字段故而有可能出现为空值的情况。
		Map<String,Object> map=new HashMap<String, Object>();
		map.put("pno", record.getpNo());
		Integer sort;
		if(archivesPoliticalInfoMapper.getsort(map)!=null&&archivesPoliticalInfoMapper.getsort(map).get("sort")!=null&&!archivesPoliticalInfoMapper.getsort(map).get("sort").equals(""))
			sort =Integer.valueOf(archivesPoliticalInfoMapper.getsort(map).get("sort").toString())+1;
		else
			sort=0;
		record.setSort(sort);
		int c= archivesPoliticalInfoMapper.insertSelective(record);
		if(c>0){
			//新增附件列表
	...
	@Override
	@Transactional
	public int updateArchivesPoliticalInfoSelective(ArchivesPoliticalInfo record,String fjparam,String ptype) {
		// TODO Auto-generated method stub
		logger.info("====开始做修改操作，修改的参数为：record"+record+"fjparam="+fjparam+"ptype="+ptype);
		int c= archivesPoliticalInfoMapper.updateByPrimaryKeySelective(record);
		if(c>0){
			ArchivesMaterialOtherInfo a=new ArchivesMaterialOtherInfo();
			if(StringUtils.isNotEmpty(ptype))
			a.setFileType(Integer.valueOf(ptype));//材料类型（1：履历材料 2：自传材料 3：鉴定考核考察材料 4：鉴定考核考察材料 5：政审材料 6：党员材料 7：奖励材料 8：处分材料9：工资变动材料 10：其他材料）
	...
	@Override
	public List<Map<String,Object>> listArchivesPoliticalInfos(
			ArchivesPoliticalInfo record) {
		// TODO Auto-generated method stub
		return archivesPoliticalInfoMapper.selectByPno(record);
	}
}
